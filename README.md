...
**Aegis radios (vanilla text chat).** Plot members can toggle Plot Frequency with `/ag chat` (or `/ag frequency`), send once with `/ag chat <message>`, and return to public chat with `/ag chat off` (`aegis.chat`). `/ag chat alliance` reaches every online alliance member; the leader can name the radio with `/ag chat alliance name <title>` (`aegis.chat.alliance`). `/ag chat group` provides the same opt-in radio for groups and leader naming (`aegis.chat.group`). Staff use `/ag staff`, `/ag staffchat`, or `/agadmin staffchat` (`aegis.admin.staffchat`). Each player can tune to only one Aegis channel at a time, which pulls their messages out of global/public chat; remaining public text continues through Hearth. Java and Bedrock players use the same commands, with no client mod required. Servers using another chat plugin can leave these channels unused or disable them in config.
