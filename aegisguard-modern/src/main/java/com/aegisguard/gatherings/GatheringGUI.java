...
public final class GatheringGUI {
    ...
    public void open(Player player) {
        ...
        inv.setItem(4, GUIManager.createItem(Material.CAMPFIRE,
                ...
                List.of(
                        ...
                        live == null ? t(player, "gathering_status_idle", "&7Not running.")
                                : t(player, "gathering_status_state", "&aLive for &f{REMAINING}",
    ...
    public void handleClick(Player player, InventoryClickEvent event, Holder holder) {
        ...
        if (slot == 11) {
            reportStart(player, service.start(player, service.minMinutes()));
            open(player);
            return;
        }
        if (slot == 12) {
            reportStart(player, service.start(player, service.defaultMinutes()));
            open(player);
            return;
        }
        if (slot == 14) {
            ...
            if (on == null) {
                if (plugin.effects() != null) plugin.effects().playError(player);
                if (plugin.msg() != null) plugin.msg().send(player, "gathering_save_failed");
                return;
            }
            if (plugin.msg() != null) {
        ...
        if (slot == 15) {
            if (service.stop(player) == GatheringService.StopResult.SAVE_FAILED) {
                if (plugin.effects() != null) plugin.effects().playError(player);
                if (plugin.msg() != null) plugin.msg().send(player, "gathering_save_failed");
            }
            open(player);
    ...
    private void reportStart(Player player, GatheringService.StartResult result) {
        if (result == GatheringService.StartResult.STARTED || result == GatheringService.StartResult.EXTENDED) return;
        if (plugin.effects() != null) plugin.effects().playError(player);
        String key = result == GatheringService.StartResult.SAVE_FAILED
                ? "gathering_save_failed" : "gathering_bad_duration";
        if (plugin.msg() != null) plugin.msg().send(player, key);
    }
