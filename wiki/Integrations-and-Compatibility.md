...
> **Upgrade note:** Existing 1.2.7 integrations remain valid in 1.3.0. New systems such as Guest Passes, Emergency Lockdown, Realm Profiles, Routes, Alliance Access, and the Staff Audit Ledger do not require additional bridge plugins.
...
| Dynmap | Web Map | Can display player and server-plot borders when map rendering is enabled. For-rent styling and realm display names are available in 1.3.0. |
...
| PlaceholderAPI | Placeholders | Use AegisGuard placeholders such as `%aegis_owner%`, `%aegis_plot%`, and `%aegis_role%` where configured. 1.3.0 expands the placeholder set. |
| LuckPerms | Permissions | Recommended permission manager for player, staff, and integration permission nodes, including `aegis.admin.routes` and `aegis.admin.audit`. |
...
1.3.0 may also create additional plugin data files for systems such as alliances and routes. Back up the full `plugins/AegisGuard/` folder and any configured SQL database before major updates, configuration changes, or migrations. A folder backup is recommended, not required, to keep claims on a 1.2.7 to 1.3.0 JAR swap.
...
These 1.3.0 systems are built into AegisGuard and do not require companion plugins:
...
| `/agadmin transition` | Confirms 1.2.7 to 1.3.0 upgrade status after a JAR swap. |
...
Before launching AegisGuard `v1.3.0` on a live server:
...
10. After upgrading from 1.2.7, run `/agadmin transition`, then verify claims, markets, Guest Passes, Lockdown, Realm Profile, Routes, and Alliance Access. Run `/agadmin doctor` only if something looks wrong.
